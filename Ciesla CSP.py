...
M_seed = [10**4.5,10**5.5,10**6.5,10**7.5,10**8.5]
#==============================================================================
# Test 
#==============================================================================
Ages = np.linspace(0.5,13.,26)* u.Gyr
...
with quantity_support():
    for k, Av in enumerate(Dust):
            for j,m in enumerate(M_seed):
                plt.plot(X[0,:,j,k,0], Y[0,:,j,k,0], label='log M$_{today}$ = %.1f'%np.log10(m),
                         lw=2, alpha=0.7)
...
for i, m in enumerate(M_seed):
    plt.plot(Ages,UV[i],label='log M$_{today}$ = %.1f'%np.log10(m))
...
if make_table:
    ...
    np.savetxt('Ciesla/Ciesla_SFH_dust.cat', data, header=' '.join(data.colnames),
