...
#    color = np.mod(5*np.pi*np.log(10**(lgAges-9))+np.pi,2*np.pi) /np.pi
    ...
    for j,(m,c) in enumerate(zip(M_today,M_color)):
        s = plt.scatter(model[age_cond&(M_class==m)], FAST[age_cond&(M_class==m)], 
                              c=c, s=15, alpha=0.5,zorder=2, label="RP%d"%(j+1))
...
plt.text(0.66,0.84,r"Av$\rm _{FAST}$", alpha=0.8,fontsize=10,transform=axins.transAxes)
...
for i, (l, model, FAST) in enumerate(zip(legends,[np.log10(10**lgAges_mw),lgMs,lgSSFR],
    ...
    for j, (m,c) in enumerate(zip(M_today,M_color)):
        s = plt.scatter(models[age_conds&(M_class2==m)], FAST[age_conds&(M_class2==m)], 
                              c=c, cmap='jet', s=8, alpha=0.3,zorder=2, label="RP%d"%(j+1))
                              #label=r'M$\rm _{today}$ = 10$^{%.1f}$'%m)
    plt.scatter(models[~age_conds], FAST[~age_conds],
...
plt.xlim(0.7,1.3)
fig.subplots_adjust(left=0.1,right=0.95,bottom=0.1,top=0.95,wspace=0.25)
#plt.savefig("N/Aldo/FAST-exp_AM-SFH_m_obs.png",dpi=350)
...
# Contour Plot Obs M
# =============================================================================
def plot_contour(X,Y,c,ax=ax,lw=3):
    color_list=[colors.to_rgba(c,alpha=alpha) for alpha in np.linspace(0.2,0.5,3)]
    my_cm = LinearSegmentedColormap.from_list(c, color_list, N=3)

    H, xbins, ybins = np.histogram2d(X, Y,bins=(np.linspace(np.percentile(X,1)-0.4, 
                                                            np.percentile(X,99)+0.4, 20), 
                                                np.linspace(np.percentile(Y,1)-0.4, 
                                                            np.percentile(Y,99)+0.4, 20)))    
    XH = np.sort(pd.Series(H[H!=0].ravel()))
    Hsum = XH.sum()
    XH_levels = [np.argmin(abs(np.cumsum(XH)-q*Hsum)) for q in [0.01,0.32,1.]]
    levels = [XH[k] for k in XH_levels]
    
    Z = gaussian_filter(H, sigma=0.6, order
                        =0).T
    
    plt.contourf(Z, levels, extent=[xbins.min(),xbins.max(), ybins.min(),ybins.max()], 
                                   norm=LogNorm(1, XH_levels[-1]), linewidths=lw,
                                   cmap=my_cm,linestyles='solid',alpha=.3,zorder=2)
    return ax
...
for i, (l, model, FAST) in enumerate(zip(legends,[np.log10(10**lgAges_mw),lgMs,lgSSFR],
    ...
    models = np.tile(model,S)
    age_conds = np.tile(age_cond,S)
    for m,c in zip(M_today[::-1],M_color[::-1]):
        X,Y = models[age_conds&(M_class2==m)], FAST[age_conds&(M_class2==m)]
        plot_contour(X,Y,c)
    #plt.scatter(models[~age_conds], FAST[~age_conds],
     #           c='gray', s=8, alpha=0.05,zorder=1,label=None)
    plt.xlabel('log'+l+r'$\rm_{Model}$',fontsize='large')
    plt.ylabel('log'+l+r'$\rm_{FAST}$',fontsize='large')
    for k,(m,c) in enumerate(zip(M_today,M_color)):
        y = pd.Series(FAST-models)[age_conds&(M_class2==m)]
...
#            fontsize=12,transform=ax.transAxes,color="gray")
    ...
    if i!=0:
        axins = inset_axes(ax, 1.4, 1.2, bbox_to_anchor=(0.98,0.44), 
                           bbox_transform=ax.transAxes)
        for m,c in zip(M_today[::-1],M_color[::-1]):
            X,Y = models[age_conds&(M_class2==m)], (FAST-models)[age_conds&(M_class2==m)],
            plot_contour(X,Y,c)
        #axins.scatter(models[~age_conds], (FAST-models)[~age_conds],
        ...
        plt.tick_params(axis='both', which='major', labelsize=8)
        if i==2:
            ax.set_xlim(-10.65,-8.15)
            ax.set_ylim(-10.65,-8.15)
            axins.set_xlim(-10.65,-8.15)
            axins.set_ylim(-.4,0.6)
        elif i==1:
            ax.set_xlim(7.35,11.55)
            ax.set_ylim(7.35,11.55)
            axins.set_xlim(7.4,11.4)
            axins.set_ylim(-.35, 0.25)
    else:
        plt.xlim(7.9,10.1)
        plt.ylim(7.9,10.1)
...
for k,(m,c) in enumerate(zip(M_today,M_color)):
    X,Y = (FAST-models)[age_conds&(M_class2==m)], Av_FAST2[age_conds&(M_class2==m)]
    plot_contour(X,Y,c,lw=2)
#    s = plt.scatter((FAST-models)[age_conds&(M_class2==m)], Av_FAST2[age_conds&(M_class2==m)],
#                            c=c,s=2, alpha=0.05,zorder=2)
    plt.text(0.05,0.92-0.08*k,'$\sigma$ = %.2f'%pd.Series(Av_FAST2-1)[age_conds&(M_class2==m)].std(),
             fontsize=9,transform=ax.transAxes,color=c)
plt.scatter((FAST-models)[~age_conds], Av_FAST2[~age_conds],
...
#plt.text(0.05,0.88,'$\sigma$ = %.2f'%pd.Series(Av_FAST-1).std(),
#         fontsize=12,transform=ax.transAxes,color="gray")
plt.xlabel('$\Delta$ log (sSFR/yr$^{-1}$)',fontsize='large')
plt.ylabel(r'$\rm Av_{FAST}$',fontsize='large')
...
for k,(m,c) in enumerate(zip(M_today,M_color)):
    color_list=[(1,1,1,0),colors.to_rgba(c,alpha=0.3),
            colors.to_rgba(c,alpha=0.5),colors.to_rgba(c)]
    my_cm = LinearSegmentedColormap.from_list(c, color_list, N=5)
    s = sns.kdeplot(models[age_conds&(M_class2==m)], FAST[age_conds&(M_class2==m)], 
                            n_levels=3,cmap=my_cm,alpha=0.5,zorder=2,
                            shade=True,shade_lowet=True)
plt.xlim(-10.6,-8.0)
plt.ylim(-10.6,-8.0)
plt.plot(xx,xx,'k--',alpha=0.75)
