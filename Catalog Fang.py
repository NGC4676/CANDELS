...
for i, zg in enumerate(zgrid):
    for j, mg in enumerate(Mgrid):
        ...
        ax.grid()
        if ((i==3) & (j==1)) | ((i==1) & (j==2)):
            plt.setp(ax.spines.values(), color="orange", linewidth=4)

for i, mg in enumerate(Mgrid):
    axes[0, i].annotate('$10^{%.1f}<$M/M$_{\odot}$$<10^{%.1f}$'%(mg-0.25,mg+0.25), 
                        (0.5, 1), xytext=(0, 10), rotation=0,
                        textcoords='offset points', xycoords='axes fraction',
                        ha='center', va='bottom', size=12)
for j, zg in enumerate(zgrid):
    axes[j, 3].annotate('$%.1f<$z$<%.1f$'%(zg-0.25,zg+0.25), 
                        (1., 0.5), xytext=(20, 0), rotation=270,
                        textcoords='offset points', xycoords='axes fraction',
                        ha='center', va='center', size=12)
fig.text(0.02,0.55,r'$\rm U - V$', rotation=90,
             ha='center', va='center', fontsize=15)
fig.text(0.5,0.11,r'$\rm V - J$', 
             ha='center', va='center', fontsize=15)
...
plt.subplots_adjust(left=0.08,right=0.92,bottom=0.15,top=0.95,wspace=0.12,hspace=0.12)
...
# Plot U-V Evolution of MW-SFG
Img_list = ["14658","22037","1742"]
fig = plt.figure(figsize=(13,6))
for i, (tg,zg,mg) in enumerate([(T1,z1,M1),(T2,z2,M2),(T3,z3,M3)]):
    print 'z:',zg,'M:',mg
    clean = (Fang.ssfr_uv_corr < Fang.ssfr_uv_corr.quantile(.975)) \
            & (Fang.ssfr_uv_corr > Fang.ssfr_uv_corr.quantile(.025))    
    cond = (abs(np.log10(Fang.M_med)-mg)<0.25) & (abs(Fang.z_best-zg)<0.15) & clean
#    SF = vUVJ_bond(VJ,UV,zg)  & (Fang.ssfr_uv_corr>-9.75)
    SF = vUVJ_bond(VJ,UV,zg)  & (abs(Fang.ssfr_uv_corr - np.log10(MS_Sch(Fang.M_med,Fang.z_best)/Fang.M_med))<0.3)

    ax = plt.subplot(1, 3, i+1)
    plt.plot(VJ_tau, UV_tau, c='m',lw=2, alpha=0.7,zorder=2)
    s = plt.scatter(VJ[cond & SF], UV[cond & SF], 
                    c=(Fang.ssfr_uv_corr - np.log10(MS_Sch(Fang.M_med,Fang.z_best)/Fang.M_med))[cond & SF],
                    s=10, cmap='rainbow_r',zorder=2,alpha=0.7)
    plt.scatter(VJ[cond & ~SF], UV[cond & ~SF],
                    s=10, c='grey',zorder=1,alpha=0.7)
    plt.scatter(VJ[cond & SF].median(), UV[cond & SF].median(), s=80, marker="o",
                facecolor='royalblue', edgecolor='k',linewidth=2,alpha=1.,zorder=3)
    plt.axhline(0.5,color='k',ls='--',alpha=0.9)
    plt.plot([-5,(0.55+0.253*zg-0.0533*zg**2)/0.88,1.6,1.6],
              [1.3,1.3,2.158-0.253*zg+0.0533*zg**2,2.5], color='k', alpha=0.7)
    plt.text(1.,0.0,r'$\bf t = %d{\ }Gyr$'%tg,fontsize=15)
    plt.text(-0.2,2.0,'z$\sim$%.1f'%Fang[cond & SF].z_best.median(),fontsize=15)
    plt.text(-0.2,1.8,'M$\sim$%.1f'%np.log10(Fang[cond & SF].M_med.median()),fontsize=15)
    plt.xlim([-0.5, 2.0])
    plt.ylim([-0.25, 2.25])
    plt.xlabel(r"$\rmV-J$",fontsize=14)
    if i==0: 
        plt.arrow(-0.3,0.72,0.673/2.,0.491/2.,fc='k', ec='k',
                lw=1, head_width=0.05, head_length=0.1)
        plt.text(-0.2,1.12,r'$\Delta$ A$_{V} = 0.5$',fontsize=10)
    if i==0: plt.ylabel(r"$\rmU-V$",fontsize=14)
    # inset image
    axins = inset_axes(ax, 1., 1., bbox_to_anchor=(0.975,0.6),  
                   bbox_transform=ax.transAxes)
    #axins.set_axis_off()
    axins.set_yticks([])
    axins.set_xticks([])
    plt.setp(axins.spines.values(), color="mediumblue", linewidth=1)
    img = mpimg.imread('N/image/GDS%s.WFC3_F160W.z%d.jpg'%(Img_list[i],(i+1)))
    axins.imshow(img)
    # colorbar
    cbar_ax = fig.add_axes([0.25, 0.1, 0.5, 0.04])
    cb = fig.colorbar(s, orientation='horizontal',cax=cbar_ax)  
# insert arrow
t_ax = fig.add_axes([0.05, 0.925, 0.9, 0.05])
t_ax.arrow(0.1, 0.6, .7, 0., fc='k', ec='k', lw = 3,
         head_width=0.6, head_length=0.03, overhang = 0.3,
         length_includes_head= True, clip_on = False)
t_ax.axis('off')
t_ax.text(0.82,0.3,r'$\bf Cosmic{\ }Time$',fontsize=18)
cb.set_label('$\Delta$ MS (dex)',fontsize=12)
plt.subplots_adjust(left=0.1,right=0.95,bottom=0.25,top=0.9,wspace=0.25,hspace=0.1)
#plt.savefig("N/MW_UVJ_evo_SFG.pdf")
plt.show()
...
for m in M_today.astype('str'):
    ...
    Data['%s'%m]  = data
z_RP,Ms_RP = Data["10.75"].z[::-1], Data["10.75"].lgMs[::-1]   
...
for i, (tg,zg,mg) in enumerate([(T1,z1,M1),(T2,z2,M2),(T3,z3,M3)]):
    ...
    cond = (abs(np.log10(Fang.M_med)-mg)<0.25) & (abs(Fang.z_best-zg)<0.15) & clean
    SF = vUVJ_bond(VJ,UV,zg)  & (abs(Fang.ssfr_uv_corr - np.log10(MS_Sch(Fang.M_med,Fang.z_best)/Fang.M_med))<0.3)
    Pop=Fang[cond & SF][(abs(UV[cond & SF]-UV[cond & SF].median())<0.1)&(abs(VJ[cond & SF]-VJ[cond & SF].median())<0.1)].sample(frac=0.5)
    print Pop.ID[Pop.field=="u"]
...
for i, (tg) in enumerate(tgs):
    ...
    clean = (Fang.ssfr_uv_corr < Fang.ssfr_uv_corr.quantile(.975)) \
            & (Fang.ssfr_uv_corr > Fang.ssfr_uv_corr.quantile(.025))    
    cond = (abs(np.log10(Fang.M_med)-mg)<0.25) & (abs(Fang.z_best-zg)<z_w) & clean
    SF = vUVJ_bond(VJ,UV,zg)
    UV_err = np.array([[UV[cond & SF].quantile(q)],[UV[cond & SF].quantile(1-q)]])
    VJ_err = np.array([[VJ[cond & SF].quantile(q)],[VJ[cond & SF].quantile(1-q)]])
    t_err = np.array([[cosmo.age(zg+z_w).value],[cosmo.age(zg-z_w).value]])
    ...
    uva = np.append(uva, UV[cond & SF].median()-UV[cond & SF].quantile(q))
    uvb = np.append(uvb, UV[cond & SF].quantile(1-q)-UV[cond & SF].median())
#    uv_bt = np.append(uv_bt,np.std([resample(UV[cond & SF]).mean() for k in range(250)]))
#    uv_bt_med = np.append(uv_bt_med,np.std([resample(UV[cond & SF]).median() for k in range(250)]))

    vjs = np.append(vjs, VJ[cond & SF].median())
    vjm = np.append(vjm, VJ[cond & SF].mean())
    vja = np.append(vja, VJ[cond & SF].median()-VJ[cond & SF].quantile(q))
    vjb = np.append(vjb, VJ[cond & SF].quantile(1-q)-VJ[cond & SF].median())
...
#    SF = vUVJ_bond(VJ,UV,zg)  & (Fang.ssfr_uv_corr>-9.75)
    ...
    uva_SF = np.append(uva_SF, UV[cond & SF].median()-UV[cond & SF].quantile(q))
    uvb_SF = np.append(uvb_SF, UV[cond & SF].quantile(1-q)-UV[cond & SF].median())
#    uv_bt_SF = np.append(uv_bt_SF,np.std([resample(UV[cond & SF]).mean() for k in range(250)]))
#    uv_bt_med_SF = np.append(uv_bt_med_SF,np.std([resample(UV[cond & SF]).median() for k in range(250)]))

    vjs_SF = np.append(vjs_SF, VJ[cond & SF].median())
    vjm_SF = np.append(vjm_SF, VJ[cond & SF].mean())
    vja_SF = np.append(vja_SF, VJ[cond & SF].median()-VJ[cond & SF].quantile(q))
    vjb_SF = np.append(vjb_SF, VJ[cond & SF].quantile(1-q)-VJ[cond & SF].median())
plt.axhline(0.5,color='k',ls='--')
#plt.axvline(3.,color='steelblue',ls='--')
#plt.axvline(7.,color='steelblue',ls='--')
plt.axvline(2.5,color='steelblue',ls='--')
...
for i, (tg) in enumerate(tgs):
    ...
    uva_a = np.append(uva_a, UV[cond & SF].median()-UV[cond & SF].quantile(q))
    uvb_a = np.append(uvb_a, UV[cond & SF].quantile(1-q)-UV[cond & SF].median())
#    uv_bt_a = np.append(uv_bt_a,np.std([resample(UV[cond & SF]).mean() for k in range(250)]))
#    uv_bt_med_a = np.append(uv_bt_med_a,np.std([resample(UV[cond & SF]).median() for k in range(250)]))

    vjs_a = np.append(vjs_a, VJ[cond & SF].median())
    vjm_a = np.append(vjm_a, VJ[cond & SF].mean())
    vja_a = np.append(vja_a, VJ[cond & SF].median()-VJ[cond & SF].quantile(q))
    vjb_a = np.append(vjb_a, VJ[cond & SF].quantile(1-q)-VJ[cond & SF].median())
