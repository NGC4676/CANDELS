...
Ages = np.linspace(0.5,11.5,111)* u.Gyr
...
plt.tight_layout()
#plt.savefig('111.png',dpi=200,bbox_inches='tight')
plt.show()
...
if make_table:
    ...
    if perturb:
        if (real_noise & multiple):    
            ...
            np.savetxt('Aldo/Perturb/Final/Aldo_SFH.cat', table, header=' '.join(table.colnames),
                       fmt=['%d']+['%.5e' for i in range(20)]+['%.2f'])
...
#                       fmt=['%d']+['%.5e' for i in range(20)]+['%.2f'])
        ...
        np.savetxt('Aldo/Aldo_SFH.cat', table, header=' '.join(table.colnames),
                   fmt=['%d']+['%.5e' for i in range(20)]+['%.2f'])
